...
server <- function(input, output, session) {
  # --- 1. 核心資料抓取與反應式物件 (穩定對接版) ---
  values <- reactiveValues(recentsearch = character(0))
  
  # 統一抓取入口：一次抓取三張表，避免多次請求被封鎖
  tmp <- reactive({
    req(input$sc)
    # 這裡會呼叫 setup.R 中的 get.data (包含 API + Chromote 策略)
    withProgress(message = '正在擷取財報數據...', value = 0.5, {
      data <- get.data(input$sc)
      if (is.null(data)) {
        showNotification("❌ 無法取得數據，請確認代碼或稍後再試", type = "error")
        return(NULL)
      }
      return(data)
    })
  ...
  # 分配數據給下游模組
  d_income_statement <- reactive({ req(tmp()); tmp()$income_statement })
  d_balance_sheet    <- reactive({ req(tmp()); tmp()$balance_sheet    })
  d_cash_flow        <- reactive({ req(tmp()); tmp()$cash_flow        })
  
  # --- 1. 截圖與基礎資料來源 ---
  screenshot_data <- eventReactive(input$search, {
    req(input$sc)
    # 顯示進度條，因為截圖需要幾秒鐘
    withProgress(message = '正在截取網頁畫面...', value = 0, {
      info <- get_screenshot_and_basic_info(input$sc)
  ...
  # --- 3. 更新 InfoBox (來源改為截圖抓取的資料) ---
  output$ibx_marketcap <- renderInfoBox({
    val <- screenshot_data()$market_cap %||% "N/A"
    infoBox("Market Cap", h3(val, style="font-weight:bold;"), icon = icon("globe"), color = "navy")
  })
  
  output$ibx_stockprice <- renderInfoBox({
    val <- screenshot_data()$price %||% "N/A"
    infoBox("Current Price", h3(val, style="font-weight:bold;"), icon = icon("tag"), color = "orange")
  })
  
  output$ibx_EPS <- renderInfoBox({
    val <- screenshot_data()$eps %||% "N/A"
  ...
  output$tbIncomeStatement <- renderDataTable({
    ...
    datatable(as.data.frame(df), 
              options = dt_options,
              rownames = FALSE)
  }, server = FALSE) # 💡 關鍵：解決 length zero 警告
  ...
  output$cf_plot <- renderPlotly({
    ...
    plot_df <- data.frame(
      Year = factor(labels, levels = rev(labels)), # 確保年份由舊到新排列
      Value = vals
    )
