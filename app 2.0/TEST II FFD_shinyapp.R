        ...
        tabBox(
          ...
          tabPanel("Stock Valuation",
                   fluidRow(
                     ...
                     column(
                       width = 6,
                       plotOutput("plt_dft_fcf")
                     )
                   )
          ),
          
          # 🧮 DCF Calculator（參數設定區）
          ...
          # 📈 g Calculator（使用 growth_module 與自訂 g）
          tabPanel("g Calculator",
                   fluidRow(
                     column(
                       width = 6,
                       growth_module_ui("growth")
                     ),
                     column(
                       width = 6,
                       h4("估算說明"),
...
server <- function(input, output, session) {
  ...
  output$tbIncomeStatement <- renderDataTable({
    d_income_statement(); print(select_clean_metric_row(d_income_statement(), "Total Revenue"))
  })
  ...
  output$tbBalanceSheet <- renderDataTable({
    d_balance_sheet(); print(select_clean_metric_row(d_balance_sheet(), "Total Assets"))
  })
  ...
  growth_result <- growth_module_server(
    id = "growth",
    d_income_statement = d_income_statement(),
    selected_industry = reactive(input$industry_choice)  # ✅ 由外部傳入產業選擇
  )
  ...
  fcf_result <- fcf_estimation_module_server(
    ...
    d_balance_sheet = d_balance_sheet,
    d_cash_flow = d_cash_flow,
    estimated_g = growth_result$g  # ✅ 傳 reactive 成長率
  )
  
  # ✅ g 改變時自動更新 UI 輸入欄位
  observeEvent(growth_result$g(), {
    g_val <- growth_result$g()
    req(g_val)
    
    updateNumericInput(session, "g_gordon", value = g_val)
    updateNumericInput(session, "g_stage1", value = g_val)
    updateNumericInput(session, "g_stage2", value = g_val)
    
    if (isTRUE(input$apply_custom_to_fcf)) {
      updateNumericInput(session, "fcf_growth_rate", value = g_val)
    }
  })
  ...
  observeEvent(input$calc, {
    ...
    if (input$dcf_mode != "gordon" && input$dcf_mode != "two_stage") {
      df_dynamic <- fcf_result$df_fcf()
      if (is.null(df_dynamic)) {
        fcf_forecast_df <- tibble(Year = NA, FCF = NA, Type = "⚠️ FCF 模型計算失敗")
      } else {
        g_val <- if (isTRUE(input$apply_custom_to_fcf)) {
          input$fcf_growth_rate
        } else {
          growth_result$g()
        }
        
        fcf_forecast_df <- df_dynamic |>
          mutate(Type = glue::glue("預設（動態模型，g = {round(g_val, 2)}%）"))
      }
      
      dcf_value <- sum(df_dynamic$FCF / (1 + wacc)^(1:years))
    }
    ...
    output$plt_fcf <- renderPlot({
      ggplot(fcf_forecast_df, aes(x = Year, y = FCF, linetype = Type)) +
        ...
        labs(title = "📉 Free Cash Flow Proj.", x = "Year", y = "FCF") +
