...
get_summary_data <- function(stock_code) {
  ...
  df <- tryCatch({
    ...
    # ==========================================
    # 🟢 修正：解析表格 (精準鎖定 Summary 區塊)
    # ==========================================
    # Yahoo 最新版的統計區塊通常被包在 data-testid='quote-statistics' 中
    list_items <- page %>% html_nodes("div[data-testid='quote-statistics'] ul > li")
    
    # 如果抓不到該特徵，退回備用方案抓取所有 li
    if (length(list_items) == 0) {
      list_items <- page %>% html_nodes("ul > li")
    }
    
    # 提取標籤與數值 (第一層 span 通常是標題，最後一層 span 是數值)
    ...
    # 🛡️ 終極防呆：白名單過濾機制
    # 只留下圖片中標示的這 16 個核心指標 (忽略大小寫與部分綴詞)
    ...
    # 過濾 DataFrame，只保留 Item 包含上述關鍵字的資料列
    parsed_df <- parsed_df[grepl(paste(target_keys, collapse = "|"), parsed_df$Item, ignore.case = TRUE), ]
    
    # 移除可能重複抓取到的隱藏元素
    parsed_df <- parsed_df[!duplicated(parsed_df$Item), ]
    # ==========================================
...
get_risk_free_rate <- function() {
  ...
  tryCatch({
    ...
    b$Page$loadEventFired() 
    Sys.sleep(3) # 給予足夠時間讓 JavaScript 渲染價格
    
    html_content <- b$Runtime$evaluate("document.documentElement.outerHTML")$result$value
    page <- rvest::read_html(html_content)
    
    # 🎯 根據你提供的 HTML 結構進行精準抓取
    # 使用屬性選擇器 [data-testid='qsp-price']
    rf_text <- page %>% 
