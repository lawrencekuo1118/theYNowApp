  ...
  dashboardSidebar(
    ...
    column(
      ...
      sidebarMenu(
        ...
        menuItem("DDM", tabName = "ddm_calculator", icon = icon("equalizer", lib = "glyphicon"), badgeLabel = "new", badgeColor = "green"),
        menuItem("DCF", tabName = "dcf_calculator", icon = icon("equalizer", lib = "glyphicon")),
  ...
  "))
    ...
    fluidRow(
      column(
        ...
        textInput("sc", "Stock Code", value = APP_DEFAULTS$stock_code),
    ...
    tabItems(
      ...
      tabItem(tabName = "ddm_calculator",
              tabBox(title = "DDM Report",
                     width = "auto",
                     
                     numericInput("ddm_app-d0", "今年發放股利 (D0)", value = APP_DEFAULTS$ddm_d0),
                     numericInput("ddm_app-g", "永續成長率 (g) %", value = APP_DEFAULTS$ddm_g),
                     numericInput("ddm_app-ke", "要求報酬率 (Ke) %", value = APP_DEFAULTS$ddm_ke),
                     
                     # 🟢 將按鈕區塊並排，加入 reset_ddm 按鈕
                     tags$div(style = "margin-top: 15px; margin-bottom: 15px;",
                              actionButton("ddm_app-btn_calc_ddm", "試算 DDM 合理股價", class = "btn-primary", icon = icon("calculator")),
                              HTML("&nbsp;&nbsp;"), # 產生一點空格
                              actionButton("reset_ddm", "回復預設", class = "btn-warning", icon = icon("refresh"))
                     )
              ),
              tags$hr(),
              
              # 輸出結果
              uiOutput("ddm_app-ui_ddm_result")
      ),
      
      # ==========================================
      # DCF Calculator 分頁 (專注於執行與結果呈現)
      # ==========================================
              ...
              tabBox(width = "auto",
                     ...
                     tabPanel(
                       fluidRow(
                         ...
                         column(
                           width = 6,
                           numericInput("years", "預測年數 n", value = APP_DEFAULTS$years, min = 1, max = 30)
                         )
                       )
                     )
              ),
                     ...
                     tabPanel("DCF Calculation Details",
                              fluidRow(
                                column(
                                  ...
                                  plotOutput(NS("mod_fcf", "fcf_plot"), height = "350px"),
              ...
              tabBox(
                ...
                tabPanel("Overview",
                         fluidRow(
                           ...
                           box(title = "DCF成長率與模型參數設定", 
                               ...
                               selectInput("g_growth_method", "歷史成長率估算方式",
                                           choices = c("學理基本面推估 (Fundamental)" = "fundamental",
                                                       "複合年均成長率 (CAGR)" = "cagr",
                                                       "平均年增率" = "mean",
                                                       "中位數年增率" = "median",
                                                       "最近一年變化" = "last_year",
                               ...
                               conditionalPanel(
                                 ...
                                 numericInput("custom_g", "自訂 g 值 (%)", value = APP_DEFAULTS$custom_g, step = 0.1)
                               ),
                               tags$hr(),
                               ...
                               conditionalPanel(
                                 ...
                                 numericInput("g_gordon", "永續成長率 g (%) [已自動綁定]", value = APP_DEFAULTS$g_gordon),
                                 numericInput("wacc_gordon", "折現率 WACC (%)", value = APP_DEFAULTS$wacc_gordon)
                               ),
                               
                               # 模型 B: 二階段成長法
                               conditionalPanel(
                                 condition = "input.dcf_mode == 'two_stage'",
                                 numericInput("yr_stage1", "第一階段預測年數", value = APP_DEFAULTS$yr_stage1, min = 1, max = 19),
                                 numericInput("g_stage1", "第一階段成長率 g₁ (%) [已自動綁定]", value = APP_DEFAULTS$g_stage1),
                                 numericInput("g_stage2", "終端永續成長率 g₂ (%) [建議採用通膨率]", value = APP_DEFAULTS$g_stage2),
                                 numericInput("wacc_stage1", "第一階段 WACC₁ (%)", value = APP_DEFAULTS$wacc_stage1),
                                 numericInput("wacc_stage2", "第二階段 WACC₂ (%)", value = APP_DEFAULTS$wacc_stage2)
                               ),
                               
                               checkboxInput("use_calculated_wacc", "✅ 採用估算 WACC 作為折現率", value = APP_DEFAULTS$use_calc_wacc),
                               tags$hr(),
                ...
                tabPanel("WACC",
                         ...
                         fluidRow(
                           ...
                           box(h4("WACC 估算"),
                               # 🟢 套用 APP_DEFAULTS
                               numericInput("wacc_re", "股權成本 rₑ (%)", value = APP_DEFAULTS$wacc_re, min = 0, step = 0.1),
                               checkboxInput("use_estimated_re", "✅ 採用估算 rₑ（來自CAPM）", value = APP_DEFAULTS$use_est_re),
                               numericInput("wacc_rd", "負債成本 rᵈ (%)", value = APP_DEFAULTS$wacc_rd, min = 0, step = 0.1),
                               numericInput("wacc_tax", "所得稅率 T (%)", value = APP_DEFAULTS$wacc_tax, min = 0, max = 100, step = 0.1),
                           ...
                           box(h4("CAPM 估算 rₑ"),
                               # 🟢 套用 APP_DEFAULTS
                               numericInput("capm_rf", "無風險利率 Rf (%)", value = APP_DEFAULTS$capm_rf, step = 0.1),
                               numericInput("capm_beta", "Beta (β) [套用產業: 預設]", value = APP_DEFAULTS$capm_beta, step = 0.1),
                               numericInput("capm_rm", "市場報酬率 Rm (%)", value = APP_DEFAULTS$capm_rm, step = 0.1),
      ...
      tabItem(tabName = "advance",
              ...
              # 用來顯示 Python 抓回來的詳細資料
              h4("Deep Scrape Result (Python Selenium)"),
              tags$hr(),
              
              fluidRow(
                column(4, actionButton("btn_expand_all", "🚀 Expand All (Deep Sync)", class = "btn-warning")),
                column(8, radioButtons("py_table_choice", "選擇報表：", 
                                       choices = c("Income Statement", "Balance Sheet", "Cash Flow"), 
                                       inline = TRUE))
              ),
              dataTableOutput("tb_python_results"),
              tags$hr(),
              ...
              tabBox(title = "Advanced FCF Settings",
                     ...
                     fluidRow(
                       box(width = 12, status = "danger", solidHeader = TRUE,
                           fluidRow(
                             column(width = 12,
                                    # ui.R 內的 DCF 頁面
                                    hr(),
                                    h4(icon("th"), "敏感度分析矩陣 (Sensitivity Analysis)"),
                                    p(helpText("💡 觀察不同 WACC 與 永續成長率 (g) 組合下，推估的每股內在價值。")),
                       ...
                       column(width = 12,
                              verbatimTextOutput("vtxt_dcf_results") # 參數設定明細
                       )
                     )
              )
      ),
      ...
      tabItem(tabName = "about",
          ...
          "))
              ...
              fluidRow(
                column(width = 12,
                       box(title = "Financial Fraud Red Flags 財務舞弊警訊", 
