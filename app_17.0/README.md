...
## v17.72 重點
- **Blue Chip／Clustering**：市值不可用時不再改依代號 A→Z 排序後再撈資料；維持原宇宙順序再取 N，再分群／分析
...
## v17.26 重點
- **Blue Chip 候選截斷**：Clustering 宇宙 N 區露出與「明細」相同的四種截斷邏輯（市值／概念股／近一年漲幅／隨機）並雙向同步；市值缺值時維持原宇宙順序（不再改依代號排序）
