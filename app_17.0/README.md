# The YNow App v17.09 — Valuation Methodology

先分類，再選模型；先推導，再校正；先給區間，再給單點；先做基本面估值。

## v17.09 重點

- **Clustering Lab**：Yahoo 比率特徵抓取韌性（Python `.info` 重試＋YfData `quoteSummary` crumb 後備＋稀疏 ticker 序列補洞；R 端 crumb 驗證並拒絕空殼；en-US／zh-TW 提示）
- **目錄**：`app_17.0/`；顯示版號 **v17.09**
...
- **目錄**：`app_17.0/`；顯示版號 **v17**（已遞增至 v17.09）
