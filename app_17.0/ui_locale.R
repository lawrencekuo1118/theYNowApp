...
.UI_STRINGS <- list(
  en = list(
    ...
    reconnect_attempting = "Attempting to reconnect",
    reconnect_try_now = "Try now",
    reconnect_banner = "Connection interrupted — reconnecting automatically (no Reload needed if the server is back).",
    reconnect_restored = "Reconnected. Restoring your inputs; Search may re-run if ticker data was cleared by a deploy.",
    refresh_link = " Refresh",
    refresh_title = "New version available — apply when ready (keeps ticker, tab, market, language, and key inputs)",
    refresh_applied = "Applied the new version and restored your workspace where possible.",
  ...
  `zh-TW` = list(
    ...
    reconnect_attempting = "正在嘗試重新連線",
    reconnect_try_now = "立即重試",
    reconnect_banner = "連線中斷 — 正在自動重新連線（伺服器恢復後通常無需按 Reload）。",
    reconnect_restored = "已重新連線。正在還原輸入；若部署清空了財報快取，Search 可能會再執行一次。",
    refresh_link = " 重新整理",
    refresh_title = "有新版本，可於方便時套用（會盡量保留代號、分頁、市場、語言與主要輸入）",
    refresh_applied = "已套用新版本，並盡可能還原您的工作狀態。",
