...
.UI_STRINGS <- list(
  en = list(
    ...
    composite_footer_note = "Blue band = main-model Bear–Bull; marker is Base. Sub-model is for cross-check only.",
    ...
    beta_advanced_note = "Peer unlever / Rolling β remain under {home} → BETA. CAPM (Rf / β / Rm) lives on DCF-Model → WACC; β source also on DCF → Beta.",
  ...
  `zh-TW` = list(
    ...
    composite_footer_note = "藍帶 = 主模型 Bear–Bull；點位為 Base。副模型僅作交叉驗證。",
    vbx_sgr_subtitle = "SGR",
    ...
    beta_advanced_note = "同業去槓桿／Rolling β 仍在 {home} → BETA。CAPM（Rf／β／Rm）在 DCF-Model → WACC；β 來源亦可於 DCF → Beta 選取。",
