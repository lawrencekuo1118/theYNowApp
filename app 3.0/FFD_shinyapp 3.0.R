...
server <- function(input, output, session) {
  ...
  # ---- Alpha Vantage financials fetcher ---- #
  library(httr)
  library(jsonlite)
  library(glue)
  
  get_financials_alpha <- function(symbol, api_key) {
  ...
  financials <- reactive({
    req(input$sc)
    tryCatch({
      get_financials_alpha(input$sc, api_key)
    }, error = function(e) {
  ...
  output$tbFinanceSummary <- renderDataTable({ financials() })
  ...
  output$ibx_marketcap <- renderInfoBox({
    req(financials())
    infoBox(title = "Market Cap.", h3(financials()[1, 4], style = "font-size:150%;"), icon = icon("money-bill-trend-up"), color = "navy")
  })
  
  output$ibx_stockprice <- renderInfoBox({
    req(financials())
    infoBox(title = "Stock Price", h3(financials()[1, 2], style = "font-size:150%;"), icon = icon("money-bill"), color = "orange")
  })
  
  output$ibx_EPS <- renderInfoBox({
    req(financials())
    infoBox(title = "EPS", h3(financials()[4, 4], style = "font-size:150%;"), icon = icon("percent"), color = "green")
  })
