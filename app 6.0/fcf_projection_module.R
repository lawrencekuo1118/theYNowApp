fcf_projection_module_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$fcf_plot <- renderPlot({
      ...
      hist_fcf <- rev(na.omit(hist_fcf_raw)) # 將時間反轉為「由舊到新」
      
      # 3. 合併資料
      if (length(hist_fcf) > 0) {
        start_proj_year <- df_proj$Year[1] # 取得預測的起始年份
      ...
      ggplot(df_plot, aes(x = Year, y = FCF, color = Type, linetype = Type)) +
        # ✅ 使用 linewidth 修正 ggplot2 棄用警告
        geom_line(linewidth = 1.2) + 
        geom_point(size = 3) +
        # ✅ 新增：在點的上方顯示具體數值 (加上千分位逗號)
        geom_text(aes(label = formatC(FCF, format = "f", big.mark = ",", digits = 0)), 
                  vjust = -1.5, size = 4.5, show.legend = FALSE, fontface = "bold") +
        
        # 🎨 自訂顏色與線條樣式，讓歷史跟預測有明確的視覺區隔
        scale_color_manual(values = c("歷史數據" = "#7f8c8d",   # 灰色代表過去
                                      "Gordon 預測" = "#2980b9", # 藍色代表未來
                                      "第一階段" = "#d35400",    # 橘色代表高成長
                                      "第二階段" = "#27ae60")) + # 綠色代表永續
        scale_linetype_manual(values = c("歷史數據" = "solid", 
                                         "Gordon 預測" = "dashed", 
                                         "第一階段" = "dashed", 
                                         "第二階段" = "dotted")) +
    ...
    return(list(
      fcf_df = fcf_df,
      hist_params = hist_params  # <-- 將這個新變數加進回傳清單
    ))
  })
}
