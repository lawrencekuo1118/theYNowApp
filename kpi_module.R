...
kpi_module_server <- function(id, d_income_statement, d_balance_sheet, d_cash_flow, industry_choice) {
  moduleServer(id, function(input, output, session) {
    ...
    output$vbx_opex_ratio <- renderValueBox({
      op_exp <- get_avg(select_clean_metric_row(d_income_statement(), "Operating Expense", include_ttm = FALSE))
      rev <- get_avg(select_clean_metric_row(d_income_statement(), "Total Revenue", include_ttm = FALSE))
    ...
    output$vbx_eqt_multiplier <- renderValueBox({
      ...
      avg_equity <- get_avg(select_clean_metric_row(d_balance_sheet(), "Common Stock Equity", include_ttm = FALSE))
    ...
    output$vbx_ROE <- renderValueBox({
      net <- get_avg(select_clean_metric_row(d_income_statement(), "Net Income from Continuing & Discontinued Operation", include_ttm = FALSE))
      equity <- get_avg(select_clean_metric_row(d_balance_sheet(), "Common Stock Equity", include_ttm = FALSE))
